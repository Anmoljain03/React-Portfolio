import { ArrowDown } from "lucide-react";

export const HeroSection = () => {
  return (
    <section
      id="hero"
      className="relative min-h-screen flex flex-col items-center justify-center px-4 text-center"
    >
      <div className="container max-w-4xl mx-auto z-10">
        <div className="space-y-6">
          <h1 className="text-4xl md:text-6xl font-bold tracking-tight opacity-0 animate-fade-in">
            Hi, I'm{" "}
            <span className="text-primary opacity-100">Anmol</span>{" "}
            <span className="text-gradient ml-2 opacity-100">Jain</span>
          </h1>

          <p className="text-lg md:text-xl text-muted-foreground mx-auto max-w-2xl opacity-0 animate-fade-in-delay-3">
            I design, build, and deploy complete web solutions — from pixel-perfect interfaces to robust back-ends.
          </p>

          <div className="pt-4 opacity-0 animate-fade-in-delay-4">
            <a href="#projects" className="cosmic-button">
              View My Work
            </a>
          </div>
        </div>
      </div>

      <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 flex flex-col items-center animate-bounce">
        <span className="text-sm text-muted-foreground mb-2">Scroll</span>
        <ArrowDown className="h-5 w-5 text-primary" />
      </div>
    </section>
  );
};
